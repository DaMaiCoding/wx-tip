...
### 2.1 数据结构
系统维护一个全局哈希表 `$global:chatHistory`：
*   **Key**: `ChatName` (聊天对象名称，如 "文件传输助手")
*   **Value**: `List<Hashtable>` (消息历史队列)
...
2.  **抓取逻辑**：
    ...
    *   提取所有非系统提示的文本消息。
    *   **合并算法**：将可视区域的消息与本地 `chatHistory` 进行比对。
        *   如果本地为空，全量存入。
...
*   **处理逻辑**：
    1.  系统立即读取 `chatHistory` 中该会话的**最后一条消息**。
...
*   **定位算法 (Context Anchor Matching)**：
    当在屏幕可视区域扫描到“撤回了一条消息”的系统提示时，系统利用**上下文锚点**在 `chatHistory` 中定位原消息：
    1.  **获取锚点**：
...
*   **实时通知**：通过标准输出 (STDOUT) 输出 JSON 格式事件，由上层应用（Electron/Node.js）消费并展示通知。
    ...
    ```
*   **日志记录**：关键操作（如同步、匹配、检测）均写入 `monitor.log` 便于排查。
