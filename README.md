...
### 3. 沉浸式弹幕通知 (Danmaku Notification)
- **原理**：在屏幕顶层覆盖一个透明窗口，将接收到的消息以弹幕形式滚动播放。
- **优势**：
  - **强提醒**：在全屏游戏或观看视频时也能第一时间看到消息，不错过重要信息。
  - **干扰低**：弹幕透明背景，鼠标穿透，不影响正常操作。
  - **可配置**：支持开关控制，随心所欲。
...
- **前端**：Electron + HTML/CSS (提供操作界面、自定义弹窗及弹幕)
- **后端**：Node.js (主进程，负责配置管理、历史记录存储及模块调度)
...
├── main/              # Electron 主进程
│   ├── index.js       # 入口文件
│   ├── modules/       # 功能模块 (Danmaku, Notification, etc.)
│   └── services/      # 核心服务
